...
class ImageSignalDataset(Dataset):
    def __init__(self, csv_path, split, image_path, signal_path, transform, original_fs, target_fs=250):
        ...
        csv_file = pd.read_csv(os.path.join(csv_path, f'csn_{split}.csv'))
        
        self.image_path = image_path
        self.signal_path = signal_path

        self.label_name = list(csv_file.columns[3:])
        self.num_classes = len(self.label_name)
        self.labels = csv_file.iloc[:, 3:].values
        
        self.file_path = list(csv_file['ecg_path'])
    ...
    def __getitem__(self, idx):
        ...
        image_path = os.path.join(self.image_path, f'{file[0]}/{file[1]}/{file[2]}-0.png')
...
def load_csn(csv_path, image_path, signal_path, fs, batch_size):
